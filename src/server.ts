...
execute: async (args, { log }) => {
 ...
 // === FILTER TOOLS ===
  server.addTool({
    name: 'setBasicFilter',
    description: 'Sets a basic filter (auto-filter) on a Google Spreadsheet sheet, enabling filter dropdowns on the header row.',
    parameters: z.object({
      spreadsheetId: z.string().describe('The ID of the Google Spreadsheet.'),
      sheetId: z.number().optional().describe('The sheet ID (from getSpreadsheetInfo). If not specified, uses the first sheet.'),
      startRowIndex: z.number().optional().describe('Start row index (0-based). Usually 0 for header row.'),
      endRowIndex: z.number().optional().describe('End row index (exclusive). Omit to include all rows.'),
    ...
    execute: async (args, { log }) => {
      const sheets = await getSheetsClient();
      log.info(`Setting basic filter on spreadsheet: ${args.spreadsheetId}`);

      const range = (args.startRowIndex !== undefined || args.endRowIndex !== undefined ||
                     args.startColumnIndex !== undefined || args.endColumnIndex !== undefined)
        ? {
      ...
      await SheetsHelpers.setBasicFilter(sheets, args.spreadsheetId, args.sheetId, range);
      return 'Basic filter set successfully. The header row now has filter dropdowns.';
    },
  });

  server.addTool({
    name: 'clearBasicFilter',
    description: 'Clears the basic filter from a Google Spreadsheet sheet.',
    parameters: z.object({
      spreadsheetId: z.string().describe('The ID of the Google Spreadsheet.'),
      sheetId: z.number().optional().describe('The sheet ID. If not specified, uses the first sheet.'),
    }),
    execute: async (args, { log }) => {
      const sheets = await getSheetsClient();
      log.info(`Clearing basic filter from spreadsheet: ${args.spreadsheetId}`);
      await SheetsHelpers.clearBasicFilter(sheets, args.spreadsheetId, args.sheetId);
      return 'Basic filter cleared successfully.';
    },
  });
...
server.addTool({
  ...
  description: 'Formats cells in a Google Spreadsheet range. Supports background color, text color, font size, bold, italic, and alignment.',
  parameters: z.object({
    spreadsheetId: z.string().describe('The ID of the Google Spreadsheet (from the URL).'),
    range: z.string().describe('A1 notation range to format (e.g., "A1:B10" or "Sheet1!A1:B10").'),
    backgroundColor: z.string().optional().describe('Background color in hex format (e.g., "#FF0000" for red, "#FFFF00" for yellow).'),
    textColor: z.string().optional().describe('Text/foreground color in hex format (e.g., "#0000FF" for blue).'),
    fontSize: z.number().int().min(1).optional().describe('Font size in points (e.g., 10, 12, 14).'),
  ...
  execute: async (args, { log }) => {
    const sheets = await getSheetsClient();
    log.info(`Formatting cells in spreadsheet ${args.spreadsheetId}, range: ${args.range}`);

    // Validate that at least one format option is provided
    const hasFormat = args.backgroundColor || args.textColor || args.fontSize !== undefined ||
                      args.bold !== undefined || args.italic !== undefined ||
                      args.horizontalAlignment || args.verticalAlignment;

    if (!hasFormat) {
      throw new UserError('At least one formatting option must be provided (backgroundColor, textColor, fontSize, bold, italic, horizontalAlignment, or verticalAlignment).');
    }
    ...
    try {
      ...
      if (args.backgroundColor) {
        const bgRgb = SheetsHelpers.hexToRgb(args.backgroundColor);
        if (!bgRgb) {
          throw new UserError(`Invalid background color format: ${args.backgroundColor}. Use hex format like "#FF0000".`);
        }
        format.backgroundColor = bgRgb;
      }

      if (args.textColor || args.fontSize !== undefined || args.bold !== undefined || args.italic !== undefined) {
        format.textFormat = {};

        if (args.textColor) {
          const textRgb = SheetsHelpers.hexToRgb(args.textColor);
          if (!textRgb) {
            throw new UserError(`Invalid text color format: ${args.textColor}. Use hex format like "#0000FF".`);
          }
          format.textFormat.foregroundColor = textRgb;
        }

        if (args.fontSize !== undefined) {
          format.textFormat.fontSize = args.fontSize;
        }
        ...
        if (args.italic !== undefined) {
          format.textFormat.italic = args.italic;
        }
      }
      ...
      if (args.backgroundColor) appliedFormats.push(`background: ${args.backgroundColor}`);
      if (args.textColor) appliedFormats.push(`text color: ${args.textColor}`);
      if (args.fontSize !== undefined) appliedFormats.push(`font size: ${args.fontSize}pt`);
      if (args.bold !== undefined) appliedFormats.push(`bold: ${args.bold}`);
      if (args.italic !== undefined) appliedFormats.push(`italic: ${args.italic}`);
      if (args.horizontalAlignment) appliedFormats.push(`horizontal align: ${args.horizontalAlignment}`);
      if (args.verticalAlignment) appliedFormats.push(`vertical align: ${args.verticalAlignment}`);

      return `Successfully formatted range ${args.range}.\nApplied formats: ${appliedFormats.join(', ')}`;
    } catch (error: any) {
      log.error(`Error formatting cells in spreadsheet ${args.spreadsheetId}: ${error.message || error}`);
      if (error instanceof UserError) throw error;
