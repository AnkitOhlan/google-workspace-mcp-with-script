  ...
  export async function setBasicFilter(
    ...
    try {
      ...
      const response = await sheets.spreadsheets.batchUpdate({
        spreadsheetId,
        requestBody: {
          requests: [{ setBasicFilter: { filter: { range: filterRange } } }],
        },
      });
      return response.data;
    } catch (error: any) {
      if (error.code === 404) throw new UserError('Spreadsheet not found.');
      if (error.code === 403) throw new UserError('Permission denied.');
      if (error instanceof UserError) throw error;
      throw new UserError('Failed to set basic filter: ' + (error.message || 'Unknown'));
    }
  }
  ...
  export async function clearBasicFilter(
    ...
    try {
      ...
      const response = await sheets.spreadsheets.batchUpdate({
        spreadsheetId,
        requestBody: {
          requests: [{ clearBasicFilter: { sheetId: targetSheetId } }],
        },
      });
      return response.data;
    } catch (error: any) {
      ...
      if (error.code === 403) throw new UserError('Permission denied.');
      if (error instanceof UserError) throw error;
      throw new UserError('Failed to clear filter: ' + (error.message || 'Unknown'));
    }
  }
